...
export function EditNoteModal({ title, body, bgColor, noteId, open, onClose }: { title?: string, body?: string, noteId?: string, bgColor: string, open: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    if (localUpdate.current) {
      updateNote(noteId!, titleInputRef.current as string, bodyInputRef.current as string, tempBgColor as string)
        .then(() => {
  ...
  // Check for tempBgColor change and set localUpdate to true
  // useEffect(() => {
  ...
  const handleClickOutside = async (e: MouseEvent) => {
    if (modalRef.current && !modalRef.current.contains(e.target as Node)) {
      ...
      if ((titleInputRef.current || bodyInputRef.current) && madeChanges.current) {
        localUpdate.current = true;
  ...
  return (
    <div className={`fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-50 ${open ? 'flex' : 'hidden'} justify-center items-center z-10`}>
      <div className={`flex flex-col ${tempBgColor} p-8 rounded-2xl min-h-96 w-2/4 drop-shadow-lg`} ref={modalRef}>
        ...
        <NoteModalFooter bgColor={tempBgColor} setbgColor={setTempBgColor} madeChanges={madeChanges} />
      </div>
    </div>
  )
}